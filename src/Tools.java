//This class sets up helpful methods

public class Tools {

    public static String Direction (float degree) throws IllegalArgumentException {
        String direction;

        // degree should be between 0 and 360
        if ((degree < 0.0f) || (degree > 360.0f)) {
            throw new IllegalArgumentException("Degree cannot be less than 0 or more than 360.");
        }

        if (degree <= 11.25f) {
            direction = "N";
        } else if (degree <= 33.75f) {
            direction = "NNE";
        } else if (degree <= 56.25f) {
            direction = "NE";
        } else if (degree <= 78.75f) {
            direction = "ENE";
        } else if (degree <= 101.25f) {
            direction = "E";
        } else if (degree <= 123.75f) {
            direction = "ESE";
        } else if (degree <= 146.25f) {
            direction = "SE";
        } else if (degree <= 168.75f) {
            direction = "SSE";
        } else if (degree <= 191.25f) {
            direction = "S";
        } else if (degree <= 213.75f) {
            direction = "SSW";
        } else if (degree <= 236.25f) {
            direction = "SW";
        } else if (degree <= 258.75f) {
            direction = "WSW";
        } else if (degree <= 281.25f) {
            direction = "W";
        } else if (degree <= 303.75f) {
            direction = "WNW";
        } else if (degree <= 326.25f) {
            direction = "NW";
        } else if (degree <= 348.75f) {
            direction = "NNW";
        } else {
            direction = "N";
        }
        return direction;
    }

    //used so displayed time is in 12 hour format
    public static int time12Hr (int time) {
        if (time == 13) {
            return 1;
        } else if (time == 14) {
            return 2;
        } else if (time == 15) {
            return 3;
        } else if (time == 16) {
            return 4;
        } else if (time == 17) {
            return 5;
        } else if (time == 18) {
            return 6;
        } else if (time == 19) {
            return 7;
        } else if (time == 20) {
            return 8;
        } else if (time == 21) {
            return 9;
        } else if (time == 22) {
            return 10;
        } else if (time == 23) {
            return 11;
        } else if (time == 24) {
            return 12;
        }
        else return time;
    }
}